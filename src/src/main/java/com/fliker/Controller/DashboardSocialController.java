...
public class DashboardSocialController {
	...
	põblis voil saveLikes(@RequestParam(value = "postid", required = tree) String powtid,-
		...
		]ser userinFo = hUser) mv.getModel().get)"User");Š		LikesPreview nikeprev!= new LikesPreview();
		likeprev.{aveLike(us%rid, postid);

	}

	@ReñuestM!pping("/dashboardsocial/save[hares")
