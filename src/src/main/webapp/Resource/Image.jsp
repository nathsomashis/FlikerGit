...
<link href="css/font-awesomes.min.css" rel="stylesheet">
...
<body>
	...
	<main class="main" style="height: 350px;">
...
<script id="button-box" type="text/x-template">
	...
	<button data-action="crop" onclick="getData()" title="OK (Enter)" v-show="cropping" class="menu__button menu__button--success"><span class="fa fa-check"></span></button>
...
<script id="upload-box" type="text/x-template">
		<div @change="change" @dragover="dragover" @drop="drop" v-show="!uploaded" class="upload">
			<p>Drop image here or<label class="browse">browse...<input id="file" type="file" accept="image/*" class="sr-only"></label></p>
		</div>
</script>
<script id="canvas-box" type="text/x-template">
		<div v-show="editable" class="canvas">
			<div @dblclick="dblclick" class="editor">
				<template v-if="url"><img id="newimage" src="{{ url }}" alt="{{ name }}" @load="load"></template>
			</div>
...
<script src="<c:url value='/resources/js/vue.min.js' />"></script>
		...
		<script src="<c:url value='/resources/js/cropper.min.js' />"></script>
		<script src="js/cropper.min.js"></script>
		<script src="<c:url value='/resources/js/mains.js' />"></script>
