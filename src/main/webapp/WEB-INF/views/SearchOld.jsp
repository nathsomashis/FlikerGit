								...
								<form action="searchresults?" id="search-form">
									...
									<div class="input-group input-group-lg hidden-mobile">
										...
										<div class="input-group-btn">
											<button type="submit" class="btn btn-default" id="searchresult">
												&nbsp;&nbsp;&nbsp;<i class="fa fa-fw fa-search fa-lg"></i>&nbsp;&nbsp;&nbsp;
											</button>
										</div>
									</div>
								</form>
								<h1 class="font-md"> Search Results for <small class="text-danger" id="searchcount"> &nbsp;&nbsp;(2,281 results)</small></h1>
								<div class="row">
								<%
									ArrayList searchresults = new ArrayList();
									if(request.getAttribute("searchResult")!= null){
										searchresults = (ArrayList)request.getAttribute("searchResult");
									}
									//System.out.println(searchresults);
									if(!searchresults.isEmpty() && searchresults.size()>0){
										...
										for(int n=0;n<searchresults.size();n++){
											...
											if(searchresults.get(n) instanceof SearchContent){
											
											SearchContent searchcontent = (SearchContent)searchresults.get(n);
											searchlink = searchcontent.getContentLink();
											content = searchcontent.getContentDescription();
											contenttype = searchcontent.getContentType();
											
											String[] totalcontents = content.split(",");
											if(contenttype.equalsIgnoreCase("Apply For Guidance")){
												for(int m=0;m<totalcontents.length;m++){
													String[] pertotalcontent = totalcontents[m].split("::");
													if(pertotalcontent[0].equalsIgnoreCase("Guidance Subject ")){
														guidancesubject = pertotalcontent[1];
													}else if(pertotalcontent[0].equalsIgnoreCase("Profile Image ")){
														profileimage = pertotalcontent[1];
													}else if(pertotalcontent[0].equalsIgnoreCase("Profile SkypeID ")){
														skypeid = pertotalcontent[1];
													}else if(pertotalcontent[0].equalsIgnoreCase("Profile Name ")){
														profilename = pertotalcontent[1];
													}else if(pertotalcontent[0].equalsIgnoreCase("Profile Email ")){
														profileemail = pertotalcontent[1];
													}else if(pertotalcontent[0].equalsIgnoreCase("Profile CurrenStatus ")){
														currentstatus = pertotalcontent[1];
													}else if(pertotalcontent[0].equalsIgnoreCase("Profile Experience ")){
														experience = pertotalcontent[1];
													}else if(pertotalcontent[0].equalsIgnoreCase("Profile Contact ")){
														contact = pertotalcontent[1];
													}
												}
												
												}
											}
												System.out.println(searchlink+content+contenttype+guidancesubject);
											%>
												<div class="search-results clearfix smart-form">
				
													<%-- <h4><i class="fa fa-plus-square txt-color-blue"></i>&nbsp;<a href="javascript:void(0);">Guidance on <%=guidancesubject%> provided by <%=profilename%></a></h4> --%>
								
													<div>
														<br>
														<%-- <div class="url text-success">
															<%=searchlink%><i class="fa fa-caret-down"></i>
															<h3 class="margin-top-0"><br><small class="font-xs"><i>Currently working as <%=currentstatus%>with experience of <%=experience%><i>Contact: <%=contact%>,<%=profileemail%></i></i></small></h3>
														</div> --%>
														<p class="description">
